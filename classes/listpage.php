		...
		foreach($this->selectedRecs as $keys) 
		{
			...
			if(!$lockRecord && @$_REQUEST["a"] == "delete" && $retval) 
			{
						...
						if( $this->tName == ADMIN_USERS )
				{
					...
					$sql = "delete from ". $grConnection->addTableWrappers( "public.seguridad_ugmembers" ) 
...
//	normal mode row count
	...
	protected static function readMainTableSettingsFromRequest( $table )
	{
		...
		if( postvalue("pageType") == "register" )
			$mainTableShortName = GetTableURL( "public.seg_usuarios" );
	...
	protected static function checkLookupPermissions( $table )
	{
		...
		if( $mainTable == "public.seg_usuarios" )
		{
			if( array_search(postvalue("field"), $lookupMainSettings->getFieldsForRegister() ) !== FALSE )
				return true;
		}
		return false;
	}

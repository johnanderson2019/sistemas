...
{	
	...
	function fillGridData() 
	{
		...
		while($data)
		{
			...
			$username = $data["usu_identificacion"];
			$groups_sate = array();
			...
			$row["username"] = runner_htmlspecialchars($username);
				$displayUserName = $data["usu_nombres"];
	...
	function fillMembers()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "UserName" )
			...
			." from ". $grConnection->addTableWrappers( "public.seguridad_ugmembers" ) 
	...
	function fillGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "GroupID" ) .", ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "public.seguridad_uggroups" ) ." order by ". $grConnection->addFieldWrappers( "Label" );
	...
	function updateUserGroups($user, $groups)
	{
		...
		$grConnection = $cman->getForUserGroups();
		// It's expected that $this->tName is equal to 'admin_members' so the page's db connection is used #9875		
		$membersWTableName = $grConnection->addTableWrappers( "public.seguridad_ugmembers" );

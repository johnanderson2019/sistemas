...
class ChangePasswordPage extends RunnerPage
{
	...
	function __construct(&$params = "")
	{
		...
		$this->pSet = new ProjectSettings("public.seg_usuarios", $this->pageType);
	...
	protected function getSQLWhere()
	{
		...
		if( $this->token )
			return " where".$this->connection->addFieldWrappers( "reset_token9" )."=". $this->connection->prepareString( $this->token );
	...
	protected function getUpdateSQL( $newpass, $where )
	{
		...
		if( $this->token )
		{
			$setPart.= ", ".$this->connection->addFieldWrappers( "reset_token9" )."=". $this->connection->prepareString( "" ).", "
				.$this->connection->addFieldWrappers( "reset_date1" )."=". $this->connection->addDateQuotes( NULL );
				
			$setPart.= ", ". $this->connection->addFieldWrappers( "usu_estado" )."=1 ";
		}
	...
	protected function changePassword()
	{
		...
		$row = $this->cipherer->DecryptFetchedArray( $qResult->fetchNumeric() ); 
	...
	protected function checkToken()
	{
		...
		$sqlSelect = "select ".$this->connection->addFieldWrappers( "reset_date1" )." from".$this->connection->addTableWrappers("public.seg_usuarios")
			." where".$this->connection->addFieldWrappers( "reset_token9" )."=". $this->connection->prepareString( $this->token );
			
		$data = $this->cipherer->DecryptFetchedArray( $this->connection->query( $sqlSelect )->fetchAssoc() );
		if( $data )
			return secondsPassedFrom( $data["reset_date1"] ) < 86400;

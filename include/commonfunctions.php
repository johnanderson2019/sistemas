...
function checkTableName($shortTName, $type=false)
{
	...
	if ("public_seg_usuarios" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("admin_users" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("public_seguridad_uggroups" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("public_org_empresa" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("public_fac_cliente" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("public_fac_producto" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("public.seg_usuarios");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="public.seg_usuarios";
	}
	...
	$strPerm = GetUserPermissions("public.seguridad_uggroups");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="public.seguridad_uggroups";
	}
	$strPerm = GetUserPermissions("public.org_empresa");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="public.org_empresa";
	}
	$strPerm = GetUserPermissions("public.fac_cliente");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="public.fac_cliente";
	}
	$strPerm = GetUserPermissions("public.fac_producto");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="public.fac_producto";
	}
	return $arr;
}
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		$sql = "select ".$gConn->addFieldWrappers( "GroupID" )
			...
			." from ". $gConn->addTableWrappers( "public.seguridad_ugmembers" )
	...
	$sql = "select ". $gConn->addFieldWrappers( "TableName" )
		...
		." from ". $gConn->addTableWrappers( "public.seguridad_ugrights" )
...
function AfterFBLogIn( $pUsername, $pPassword, $pDisplayUsername, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("public.seg_usuarios")
...
{
	global $globalEvents;
	$_SESSION["GroupID"] = $data["usu_identificacion"];
